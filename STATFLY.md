...
**Current file:** `index.html` (cache `scorefly-v59`)
...
- **Solid:** 8-state colour system, momentum/hysteresis, comeback + cold, Fly Time + Overtime, FlyTime Buzz (new copy + batching + cooldown + tap-to-section, v53), pinned “Worth watching now” feed section + static badge (v52), after-match Fly stamp (v54), Fly Mode rendering (Split Tiles layout with per-side state washes, v58), NRL support.
...
Steps: edit in GitHub web UI, commit, wait ~30s for Pages rebuild, refresh on phone. **Every deploy that touches HTML/CSS/JS/icons: bump `CACHE` in `sw.js` to a higher number.** Current: **`scorefly-v55`**. For an installed PWA, removing + re-adding the Home Screen icon forces a stale cache to clear.
