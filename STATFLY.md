...
**Current file:** `index.html` (cache `scorefly-v54`)
...
When any live game is in Fly Time, `renderHome` lifts those matches out of the live list into a pinned green-headed section (`#flytime-section` / `#flytime-cards`) at the top of the Feed. Hidden entirely when nothing is in Fly Time. Also the FlyTime Buzz tap destination. Per-card indicator is a **static** green `🪰 FLYTIME` badge (`flyTimeBannerHTML` / `.flytime-banner`); the old scrolling marquee was removed in v52.

### After-match Fly stamp (v54)

Every match the app observes entering Fly Time is recorded id->timestamp in `flyTimeMatches` (`scorefly_flytime_matches`), pruned past 35 days. When such a match appears on Results, `buildResultCard` shows a small green `🪰 FlyTime` stamp (`.fly-stamp`). The match id is the ESPN event id, stable from live to final. Limitation: only stamps matches the app actually watched go live (not retroactive, per-device) — full historical stamping is the parked FlyTime learning system (native track).

### Functions / where to look

`clockToSec`, `detectOT`, `isFlyTime`, `momTier`, `resolveSide`, `updateFlyState(m)`, `getFlyClass(matchId, side)`, `getMatchFly(matchId)`. Notifications: `fireFlyTimeAlert` (queues), `flushFlyTimeAlerts` (batches + sends), `flyTimeNotifClick` (tap target). After-match stamp: `markFlyTimeMatch` / `matchHadFlyTime`. Live match objects carry `period`, `clockSec`, `clockRaw`, `isOT`, `hInt`, `aInt`. `applyScoreFlashes(newLive)` flashes changed cells, calls `updateFlyState`, and prunes state for matches no longer live — runs every poll.

### Status

- **Solid:** 8-state colour system, momentum/hysteresis, comeback + cold, Fly Time + Overtime, FlyTime Buzz (new copy + batching + cooldown + tap-to-section, v53), pinned “Worth watching now” feed section + static badge (v52), after-match Fly stamp (v54), Fly Mode rendering, NRL support.
- **Needs live testing:** all momentum thresholds are reasoned defaults, not yet verified against real matches; tuning on live match days is the main remaining FlyState task. The v51-v54 FlyTime work (buzz copy/batching, feed section, stamp) has passed static checks but not yet been seen against a real live Fly Time game.
- **Permanently blocked by ESPN data:** Must Watch (80-point threshold unreachable), Gold Glow. Pre-match Fly stamp (prediction) and the FlyTime learning system are the same class — parked for the native/back-end track.
...
`RIVALRIES_RAW` — hand-curated rival pairs in `"Team|League"` format, symmetric (both directions built at load time into `RIVALRIES` map). Used in onboarding suggestion chips and the skull marker in Fly Mode.
...
Steps: edit in GitHub web UI, commit, wait ~30s for Pages rebuild, refresh on phone. **Every deploy that touches HTML/CSS/JS/icons: bump `CACHE` in `sw.js` to a higher number.** Current: **`scorefly-v54`**. For an installed PWA, removing + re-adding the Home Screen icon forces a stale cache to clear.
...
|Worth watching now           |Pinned FlyTime section at top of Feed; static green badge replaced scrolling marquee (v52)                                                                        |
|After-match Fly stamp        |Result cards stamped if the app saw the match enter FlyTime; per-device, 35-day prune, not retroactive (v54)                                                      |
|Rivalries                    |Hand-curated pairs; skull marker in Fly Mode; bonus in onboarding suggestions                                                                                     |
...
**v54** (current) — After-match Fly stamp: matches the app sees enter Fly Time are recorded (`scorefly_flytime_matches`, 35-day prune) and stamped on Results via `buildResultCard`.
