...
**Current file:** `index.html` (cache `scorefly-v68`)
...
Repo files: `index.html`, `sw.js`, `manifest.json`, `icon192.png`, `icon512.png`, `fly-green.png`, `fly-yellow.png`, `fly-red.png`, `onboard-hero.png`, `README.md`, `NOTES.md`, `.nojekyll`.

Steps: edit in GitHub web UI, commit, wait ~30s for Pages rebuild, refresh on phone. **Every deploy that touches HTML/CSS/JS/icons: bump `CACHE` in `sw.js` to a higher number.** Current: **`scorefly-v68`**. For an installed PWA, removing + re-adding the Home Screen icon forces a stale cache to clear.
...
**v68** (current) — Single-hero onboarding + household-list expansion + local-pick prominence. (1) Onboarding now opens on one full-bleed hero screen (`onboard-hero.png`, JRod’s “We’ll Tell You When It Matters” poster, bottom-cropped so a sticky green “Choose My Teams” button sits under it) -> team selection -> notifications. Retired the old welcome + “Meet FlyTime” intro screens (`onb-step-fly` left orphaned). Confirmed notifications come after team selection and teams are followed bell-off until permission is granted. (2) `MAJOR_CLUBS` expanded with ~50 curated household names (more EPL/Serie A/Ligue 1/Eredivisie/Primeira/Scottish soccer, more NBA/NFL/MLB/NHL, AFL/NRL clubs, A-League/BBL/NBL, IPL) so “same country” picks lean on recognition; ~21% of teams now flag major (over-flags on single-word names like “Blues”/“Warriors” are pre-existing contains-matcher behaviour, left as-is). (3) Local picks (Positions 1-2) now walk `METRO_TEAMS` in authored prominence order via a new `keepOrder` flag on `take()`, so the marquee local team surfaces (Knicks -> Yankees+Rangers, not Giants+Mets). New asset `onboard-hero.png` pre-cached; `sw.js` bumped v67->v68. Static checks pass; engine re-verified against anchors.
